...
async def send_user_data(state: FSMContext):
    ...
    async with state.proxy() as quiz_responses:
        quiz_responses.pop(QuizResponses.service_data.last_message, None)
        quiz_responses.pop(QuizResponses.service_data.editing_field, None)
        quiz_responses.pop(QuizResponses.service_data.is_editing, None)
